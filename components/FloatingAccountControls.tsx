...
export default function FloatingAccountControls({
  ...
  history?: { isOpen: boolean; onToggle: () => void };
  ...
  // 뷰포트 폭이 기준 미만이면 십자(축소) 모드
  useEffect(() => {
    if (collapseBelow == null) {
      setNarrow(false);
      return;
    }
    const mql = window.matchMedia(`(max-width: ${collapseBelow - 1}px)`);
  ...
  const collapsible = isFullscreen || narrow;

  // 접힘 조건 진입 → 십자(축소), 해제 → 패널 유지
  useEffect(() => {
    setExpanded(!collapsible);
  }, [collapsible]);

  // 컴포넌트 밖 클릭 시 접기 (접힘 조건 + 펼쳐진 상태에서만)
  useEffect(() => {
  ...
  return (
    <div
      ref={ref}
      className={`${position === 'fixed' ? 'fixed' : 'absolute'} right-6 top-6 z-50`}
      onBlur={(e) => {
        // 포커스가 컴포넌트 밖으로 나가면 접기
        if (collapsible && ref.current && !ref.current.contains(e.relatedTarget as Node)) {
          setExpanded(false);
        }
      }}
    >
      {/* 십자(+) 토글 — 접힘 조건 & 접힘일 때 노출 */}
      {collapsible ? (
        <button
          type="button"
          aria-label="계정 메뉴 열기"
      ...
      <div
        className={`flex origin-top-right items-center gap-1.5 rounded-full p-1.5 transition-all duration-300 ease-out ${GLASS} ${
          collapsible && !expanded ? 'pointer-events-none scale-50 opacity-0' : 'scale-100 opacity-100'
        }`}
      >
        {/* 사용 히스토리 열기/닫기 (아이콘) */}
        {history ? (
          <button
            ...
            className={`flex h-9 w-9 items-center justify-center rounded-full transition hover:bg-white/50 hover:text-[#10b981] ${
              history.isOpen ? 'bg-white/60 text-[#10b981]' : 'text-[#111827]/80'
            }`}
          >
        ...
        {icons?.map((item) => (
          <button
            ...
            className={`flex h-9 w-9 items-center justify-center rounded-full transition hover:bg-white/50 hover:text-[#10b981] ${
              item.active ? 'bg-white/60 text-[#10b981]' : 'text-[#111827]/80'
            }`}
          >
            {item.icon}
          </button>
        ...
        {items.map((item) => (
          <button
            ...
            className={
              item.emphasized
                ? 'rounded-full bg-white/35 px-5 py-2 text-sm md:text-base font-semibold text-[#111827]/90 shadow-[inset_0_1px_0_rgba(255,255,255,0.6)] transition hover:bg-white/60 hover:text-[#10b981]'
                : 'rounded-full px-5 py-2 text-sm md:text-base font-semibold text-[#111827]/90 transition hover:bg-white/50 hover:text-[#10b981]'
            }
          >
            {item.label}
          </button>
