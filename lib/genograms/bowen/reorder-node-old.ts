export function reorderDisplayOrders_willRm(data: BWGenogramData): BWGenogramData {
  ...
  allLevels.forEach(level => {
    ...
    console.log(`currentLevelArray : ${currentLevelArray}`)
    const missingNodes = nodes
      ...
      if (relLevel < maxSizeOfLevel) {
        ...
        sameRowNodes.forEach((node) => {
          ...
          if (family && family.parent_ids.length >= 2) {
            ...
            let countOfGenerationDepthOfAcquaintance = 0;
            if (p1 && p2) {
              ...
              if (hasMemberInBetween) {
                // 부모 지인의 자녀/후손 세대 카운트 확인
                const checkLevelMembersChildInBetween = (targetID: string): number => {
                  // 1. 특정인의 id로 자녀가 있는 가족단위를 찾음
                  const familyUnit = data.familyUnits.find(
                    (p) => p.parent_ids.includes(targetID) && p.children_ids.length > 0
                  );
                  // 자녀가 없으면 현재 세대인 1을 반환 (종료 조건)
                  if (!familyUnit) {
                    return 1;
                  }
                  // 2. 자녀들의 후손 세대 수를 각각 재귀적으로 확인
                  const childDepths = familyUnit.children_ids.map((childID) =>
                    checkLevelMembersChildInBetween(childID)
                  );
                  // 자녀들 중 가장 큰 세대수 값에 현재 세대(+1)를 더해서 반환
                  return 1 + Math.max(...childDepths);
                };
                const memberIdArrayInBetween = Array.from(
                  new Set( updatedNodes.filter( p => 
                    (p.relLevel === p1.relLevel || p.relLevel === p2.relLevel) &&
                    (p.display_order > minOrder && p.display_order < maxOrder)
                  ))
                ).map(p => p.id);
                const memberDepth = memberIdArrayInBetween.map(id => checkLevelMembersChildInBetween(id));
                countOfGenerationDepthOfAcquaintance = Math.max(...memberDepth) - 1;
              }
            }
            console.log(`node id : ${node.id}:: hasMemberInBetween::${hasMemberInBetween}`)
            console.log(`countOfGenerationDepthOfAcquaintance = ${countOfGenerationDepthOfAcquaintance}`);
            const pPos1 = positionMap[family.parent_ids[0]];
            const pPos2 = positionMap[family.parent_ids[1]];
            console.log(`pPos1 : {${pPos1.x},${pPos1.y}}, pPos2 : {${pPos2.x},${pPos2.y}}`);
            if (pPos1 && pPos2) {
              ...
              console.log("familyCenterX: ", familyCenterX);
              // 형제자매가 홀수인지 짝수인지 확인
              ...
              if (childrenCount === 1) {
                positionMap[node.id] = {
                  x: familyCenterX, 
                  y: !hasMemberInBetween ? y : countOfGenerationDepthOfAcquaintance * 160 + y
                };
              } else {
                const childrenCenterOrder = (childrenCount+1) / 2;
                const birthOrder = node.attributes.birth_order;
                const birthOrderGap = birthOrder - childrenCenterOrder;
                const childrenOffset = 
                (isOdd||childrenCount) ? 0 : (birthOrderGap > 0 ? halfNodeArea : -halfNodeArea);
                const childrenOrderGap = birthOrderGap*nodeArea + childrenOffset;
                
                console.log(`1 childrenCenterOrder: ${childrenCenterOrder}`);
                console.log(`2 birthOrderGap:${birthOrderGap}\n3 halfNodeArea : ${halfNodeArea}`);
                console.log(`4 childrenOffset: ${childrenOffset}`);
                console.log(`5 id: ${node.id}\n6 gap : ${childrenOrderGap}`);
