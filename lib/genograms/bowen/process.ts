...
import { resolveAnonymousNames } from './prune-anonymous';
 ...
 * (익명 노드 이름을 관계 지칭으로 변환 → relLevel 정규화 → display_order 정렬 → 레이아웃 좌표 계산)
 */
export function processGenogramData(raw: BWGenogramData): BWGenogramData {
  const resolved = resolveAnonymousNames(raw);
  const normalized = normalizeRelLevels(resolved);
